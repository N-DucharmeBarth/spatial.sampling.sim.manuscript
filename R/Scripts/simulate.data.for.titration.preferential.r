...
for(i in 1:10)
{
	for(j in 1:length(weight.vec))
	{
		...
		save(samp.dt,file=paste0("SimData/Titration_Preferential/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
...
# Plot examples
    ...
    ggplot2::ggsave(filename=paste0("titration.preferential.effort.distribution.png"), plot = p, device = "png", path = "Plots/",
