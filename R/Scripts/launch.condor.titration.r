...
launch_machine.stem = "/home/nicholasd/spatial.sampling.sim.manuscript/Titration/"
...
reps = 1:80 # which reps to use to calculate the indices
		...
		# grab sim data and move to loading dock
			file.copy(from=paste0(project.dir,"SimData/Titration/",list.files(paste0(project.dir,"SimData/Titration/"))[reps]), to=condor.loading.dock.dir, overwrite = TRUE, recursive = FALSE, copy.mode=TRUE)
		...
		# create new directories for results: local & remote
			dir.create(paste0("Index/Titration/"),recursive=TRUE,showWarnings=FALSE)

		# send files to remote: condor_*.sub, samp.dt.names.txt, samp.dt.*.RData, runR_*.bat, Start.tar.gz
       		scp_upload(session,files=paste0(condor.loading.dock.dir,c("condor_titration.sub","titration.samp.dt.names.txt","runR_titration.bat","Start.tar.gz",list.files(paste0(project.dir,"SimData/Titration/"))[reps])),to=paste0(launch_machine.stem,"/"))
