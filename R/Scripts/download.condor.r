...
for(q in c("noQ","Q"))
{
	for(s in c("Contraction", "Expansion", "Fixed", "Preferential", "Random", "Rotating"))
	{
		# build sub directory for log files & indices
			dir.create(paste0("Index/Simple120_wReplacement/","/",s,"_",q,"/log"),recursive=TRUE,showWarnings=FALSE)
			dir.create(paste0("Index/Simple120_wReplacement/","/",s,"_",q,"/vast"),recursive=TRUE,showWarnings=FALSE)
		...
		# download *.vast_list.RData
			...
			for(i in 1:length(download.var)){scp_download(session,files=paste0(paste0(launch_machine.stem,'/',s,'_',q),'/',download.var[i]),to= paste0("Index/Simple120_wReplacement/","/",s,"_",q,"/vast"))}

		# download *.log files
			download.var = grep("condor_R.log",var.vec,value=TRUE)
			for(i in 1:length(download.var)){scp_download(session,files=paste0(paste0(launch_machine.stem,'/',s,'_',q),'/',download.var[i]),to= paste0("Index/Simple120_wReplacement/","/",s,"_",q,"/log"))}
