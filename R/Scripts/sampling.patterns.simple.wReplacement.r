...
# 11/01/2019
# Sample the baseline abundance data WITH REPLACEMENT under 6 different spatial sampling scenarios
...
rand.samp = function(data.dt, ts.sampled = c(1,40), n.samp = 20000, cv = 0.15, random.seed = 123)
{
	...
	# sample rows
		tmp.dt = tmp.dt[sample(1:nrow(tmp.dt),size=n.samp,replace=TRUE,prob=prob.vector),][order(ts,lon,lat)]
...
for(i in 1:100)
{
	...
	# RandomZero
		...
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/RandomZero/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
	# Random
		samp.dt = rand.samp(data.dt, ts.sampled = c(1,120), n.samp = 60000, cv = 0.15, random.seed = i)
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/Random/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
	# Fixed
		samp.dt = fixed.seasonal.closure.samp(data.dt, ts.sampled = c(1,120), n.samp = 60000, cv = 0.15, random.seed = i)
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/Fixed/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
	# Rotating	
		samp.dt = rotate.seasonal.closure.samp(data.dt, ts.sampled = c(1,120), n.samp = 60000, cv = 0.15, random.seed = i)
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/Rotating/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
	# Preferential	
		samp.dt = pref.samp(data.dt, ts.sampled = c(1,120), n.samp = 60000, cv = 0.15, random.seed = i)
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/Preferential/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
	# Expansion	
		samp.dt = expand.samp(data.dt, ts.sampled = c(1,120), n.samp = 60000, cv = 0.15, random.seed = i)
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/Expansion/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
	# Contraction	
		samp.dt = contract.samp(data.dt, ts.sampled = c(1,120), n.samp = 60000, cv = 0.15, random.seed = i)
		save(samp.dt,file=paste0("SimData/Simple120_wREPLACEMENT/Contraction/samp.dt.",save.id,".RData"))
		rm(list=c("samp.dt"))
}
