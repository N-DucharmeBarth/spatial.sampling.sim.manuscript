...
launch_machine.stem = "/home/nicholasd/spatial.sampling.sim.manuscript/Titration_Preferential_wReplacement/"
...
reps = 1:80 # which reps to use to calculate the indices
		...
		# grab sim data and move to loading dock
			file.copy(from=paste0(project.dir,"SimData/Titration_Preferential_wReplacement/",list.files(paste0(project.dir,"SimData/Titration_Preferential_wReplacement/"))[reps]), to=condor.loading.dock.dir, overwrite = TRUE, recursive = FALSE, copy.mode=TRUE)
		...
		# grab files and move to loading dock: condor_*.sub, samp.dt.names.txt, runR_*.bat, condor_run.*.r, data.dt.RData, fn.add.catchability.r, nino.df.RData, r361port.tar.gz, rm_except, simple.true.index.RData, sst.storage.df.RData, VAST_v8_3_0.*
			...
			file.copy(from=paste0(project.dir,"R/Scripts/",c("condor_run.titration.wReplacement.r")), to=condor.loading.dock.dir, overwrite = TRUE, recursive = FALSE, copy.mode=TRUE)
			file.rename(from=paste0(condor.loading.dock.dir,"condor_run.titration.wReplacement.r"),to=paste0(condor.loading.dock.dir,"condor_run.titration.r"))
		...
		# create new directories for results: local & remote
			dir.create(paste0("Index/Titration_Preferential_wReplacement/"),recursive=TRUE,showWarnings=FALSE)

		# send files to remote: condor_*.sub, samp.dt.names.txt, samp.dt.*.RData, runR_*.bat, Start.tar.gz
       		scp_upload(session,files=paste0(condor.loading.dock.dir,c("condor_titration.sub","titration.samp.dt.names.txt","runR_titration.bat","Start.tar.gz",list.files(paste0(project.dir,"SimData/Titration_Preferential_wReplacement/"))[reps])),to=paste0(launch_machine.stem,"/"))
