...
# create list of rasters (avg sst by year x quarter)
	...
	time.seq = seq(ISOdate(1854,1,1), ISOdate(2018,12,1), "months")[1153:1980]
	yrqtr.seq = sapply(time.seq,function(x)paste0(substr(x,1,4),0,ifelse(as.numeric(substr(x,6,7))<=3,1,ifelse(as.numeric(substr(x,6,7))<=6,2,ifelse(as.numeric(substr(x,6,7))<=9,3,4)))))
...
# save the output
	save(sst.ras.list,file="C:/Users/nicholasd/Desktop/SPC/SPC_SAM/Geostats/sst.ras.list.RData")
	save(sst.storage.df,file="C:/Users/nicholasd/Desktop/SPC/SPC_SAM/Geostats/sst.storage.df.RData")
